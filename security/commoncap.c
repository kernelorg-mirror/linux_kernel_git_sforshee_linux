...
/**
 * vfs_caps_xattr_size - get xattr size of vfs_caps
 *
 * @vfs_caps:   destination buffer for vfs_caps data
 *
 * Returns the size of the xattr representation of @vfs_caps, or <0
 * on error.
 */
ssize_t vfs_caps_xattr_size(const struct vfs_caps *vfs_caps)
{
...
EXPORT_SYMBOL(vfs_caps_from_xattr);
				   ...
				   void *data, size_t size)
{
	...
	rootid = 0;
	xattr_size = vfs_caps_xattr_size(vfs_caps);
	if (xattr_size < 0)
		return xattr_size;
	if (size < xattr_size)
		return -EINVAL;
	...
	if ((vfs_caps->magic_etc & VFS_CAP_REVISION_MASK) == VFS_CAP_REVISION_3) {
...
EXPORT_SYMBOL(vfs_caps_to_xattr);
