			      ...
			      const struct vfs_caps *caps, int flags)
{
	if (unlikely(IS_PRIVATE(d_backing_inode(dentry))))
		return 0;
	return call_int_hook(inode_set_fscaps, idmap, dentry, caps, flags);
}
...
int security_inode_get_fscaps(struct mnt_idmap *idmap, struct dentry *dentry)
{
	if (unlikely(IS_PRIVATE(d_backing_inode(dentry))))
		return 0;
	return call_int_hook(inode_get_fscaps, idmap, dentry);
}
...
int security_inode_remove_fscaps(struct mnt_idmap *idmap, struct dentry *dentry)
{
	if (unlikely(IS_PRIVATE(d_backing_inode(dentry))))
		return 0;
	return call_int_hook(inode_remove_fscaps, idmap, dentry);
}

/**
 * security_inode_post_remove_fscaps() - Update the inode after removing fscaps
 * @idmap: idmap of the mount
 * @dentry: file
 *
 * Update inode security field after successfully removing fscaps.
 *
 */
void security_inode_post_remove_fscaps(struct mnt_idmap *idmap,
				       struct dentry *dentry)
{
	if (unlikely(IS_PRIVATE(d_backing_inode(dentry))))
		return;
	call_void_hook(inode_post_remove_fscaps, idmap, dentry);
}
